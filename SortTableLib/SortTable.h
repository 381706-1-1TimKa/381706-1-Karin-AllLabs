...
TSortTable<T>::TSortTable(TString* keys, T* dataes, int _size, int sort)
{
	...
	case 3:
		...
		break;
	default:
		throw TException("incorect sort value", "SortTable", "TSortTable", 2);
	}
}
...
int TSortTable<T>::Put(TString k, T d)
{
	...
	while (right >= left)
	{
		...
		else if (mas[cur].GetKey() < k)
		{
			...
			throw TException("Element already exist", "SortTable.h", "Put", 3);
	}
...
void TSortTable<T>::Del(TString k)
{
	...
	while (right >= left)
	{
		...
		throw TException("Element isn't find", "SortTable", "Delete", 4);
}
...
{
	if (s < this->count)
		throw TException("New size is less then number of elements", "SortTable.h", "Resize", 5);
	if (s == this->size)
		return;
...
TSortElem<T>& TSortTable<T>::operator[](TString k)
{
	...
	if (A == not_find)
		throw TException("Element isn't find", "SortTable", "operator[]", 4);
	return A;
}
...
{
	if ((i >= count) || (i < 0))
		throw TException("Value isn't correct", "SortTable", "operator[](int)", 6);
	return mas[i];
}
